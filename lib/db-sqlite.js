...
function ensureCategoriesColumns(db) {
...
function ensureServicesColumns(db) {
  ensureColumns(db, "services", [
    ["description", "TEXT NOT NULL DEFAULT ''"],
    ["ideal_for", "TEXT NOT NULL DEFAULT ''"],
    ["deliverables", "TEXT NOT NULL DEFAULT ''"],
    ["cta", "TEXT NOT NULL DEFAULT ''"],
...
function seedUsers(db) {
  ...
  db.prepare(
    ...
    `,
  ).run(username, hashPassword(password), displayName, email);
}
...
function seedSiteSettings(db) {
  ...
  db.prepare(
    ...
    DEFAULT_SITE_SETTINGS.logoPath,
    DEFAULT_SITE_SETTINGS.logoAlt,
...
function seedCategoriesAndServices(db) {
...
function toSettings(row) {
  return {
    ...
    logoPath: withDefault(row.logo_path, DEFAULT_SITE_SETTINGS.logoPath),
    logoAlt: withDefault(row.logo_alt, DEFAULT_SITE_SETTINGS.logoAlt),
    ...
    navServicesLabel: withDefault(row.nav_services_label, DEFAULT_SITE_SETTINGS.navServicesLabel),
    ...
    navContactCtaLabel: withDefault(
      row.nav_contact_cta_label,
      DEFAULT_SITE_SETTINGS.navContactCtaLabel,
    ),
...
function uniqueTableSlug(db, table, baseSlug, excludeId) {
...
function uniqueSlug(db, baseSlug, excludeId) {
  return uniqueTableSlug(db, "projects", baseSlug, excludeId);
}
...
export function listCategories({
  ...
  const services = includeServices || includeAnalytics
  ...
  return rows.map((row) => {
    const categoryServices =
      includeServices || includeAnalytics
        ? services.filter((service) => service.categoryId === row.id)
        : [];
    ...
    return toCategory(row, categoryServices, analytics);
  });
}
...
export function getCategoryById(id, options = {}) {
  ...
  const services = options.includeServices
    ? listServices({ includeArchived: true, categoryId: id })
    : [];
...
export function getServiceById(id) {
  const db = getDbInstance();
  const row = db.prepare("SELECT * FROM services WHERE id = ?").get(Number(id));
...
export function updateSiteSettings(input) {
  ...
  const values = {
    ...
    logoAlt: input.logoAlt?.trim() || DEFAULT_SITE_SETTINGS.logoAlt,
    ...
    navServicesLabel:
      input.navServicesLabel?.trim() || DEFAULT_SITE_SETTINGS.navServicesLabel,
    ...
    navContactCtaLabel:
      input.navContactCtaLabel?.trim() || DEFAULT_SITE_SETTINGS.navContactCtaLabel,
...
export function saveCategory(input) {
  ...
  const current = categoryId ? getCategoryById(categoryId) : null;
  const name = input.name?.trim() || "Untitled category";
  const values = {
    slug: uniqueTableSlug(db, "categories", input.slug || name, categoryId),
    name,
...
export function saveService(input) {
  ...
  const current = serviceId ? getServiceById(serviceId) : null;
  const name = input.name?.trim() || "Untitled service";
  const values = {
    categoryId: Number(input.categoryId) || current?.categoryId || null,
    slug: uniqueTableSlug(db, "services", input.slug || name, serviceId),
    name,
    description: input.description?.trim() || "",
...
export function deleteCategory(id, replacementCategoryId = null) {
  const db = getDbInstance();
  const categoryId = Number(id);
  const current = getCategoryById(categoryId, { includeServices: true });
  if (!current) return null;
  ...
  if (replacementId && replacementId !== categoryId) {
    ...
    db.prepare("DELETE FROM services WHERE category_id = ?").run(categoryId);
    db.prepare(
      `
        UPDATE projects
...
export function bulkUpdateCategoryStatus(ids, status) {
  const db = getDbInstance();
...
export function getCategoryAnalytics() {
  ...
  const services = listServices({ includeArchived: true });
