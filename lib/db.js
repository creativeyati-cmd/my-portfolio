...
async function getFallbackDriver() {
  return import("./db-json");
}
...
async function call(method, args) {
  ...
  try {
    ...
    const fallback = await getFallbackDriver();
    if (fallback === driver) throw error;

    console.error(
      `[db] ${method} failed on primary driver in Vercel runtime, falling back to JSON storage.`,
      error,
    );
